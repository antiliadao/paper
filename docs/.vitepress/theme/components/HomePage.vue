    ...
    <div class="content-wrapper">
      ...
      <section class="hero-section">
        <div class="hero-content">
          ...
          <h2 class="hero-title">
            <span v-if="t.title.main" class="title-zh">{{ t.title.main }}</span>
            <span v-if="t.title.main" class="title-divider">·</span>
            <span class="title-contract">{{ t.title.sub }}</span>
          </h2>
...
const currentLang = computed(() => {
  ...
  return 'en-US' // English is now the default at root
})
...
const localePath = (path) => {
  ...
  if (routePath.includes('/zh-CN/')) langPrefix = '/zh-CN'
  ...
  // No prefix for English (root)
  
  return `/paper${langPrefix}${path}`.replace(/\/+/g, '/')
}
